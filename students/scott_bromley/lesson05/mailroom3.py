...
def send_thank_you():
    ...
    if donor_name == 'list':
        ...
        try:
            ...
            if bool(donation) is False:
                raise ValueError(f"Invalid donation amount: {donation}\n")
        except ValueError as donation_err:
            raise donation_err
        else:
            donor_info = {"first_name": donor_name.strip().split(' ')[0], "last_name":
                          donor_name.strip().split(' ')[1], "donation_amount": donation}
        finally:
...
# def safe_input(func):
#     """
#     wrapper to get_user_input
#     :param func: function
#     :return: None for KeyboardInterrupt and EOF exceptions
#     """
#     def safe_input_wrapper(prompt):
#         try:
#             input(prompt).strip()
#         except (KeyboardInterrupt, EOFError):
#             return None
#     return safe_input_wrapper
