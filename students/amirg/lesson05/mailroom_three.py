...
def createareport():
    ...
    print(top_string + ''.join(middle_string))
...
def sub_response(action, action_two, dictionary):
    ...
    try:
        ...
        return sub_response(name_input, donation_input, donors)
...
def write_donors(x, y):
    ...
    with open(x + '.txt', 'w+') as f:
        f.write(paragraph.format(x,y[-1]))
    f.close()


def all_donors():
    '''Writes all the donor letters to a file in {name}.txt format'''
    [write_donors(key, value) for key, value in donors.items()]
