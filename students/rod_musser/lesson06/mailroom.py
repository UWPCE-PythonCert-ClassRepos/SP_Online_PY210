...
def list_donors():
    ...
    """
    newLine = '\n'
    return f"{newLine.join(donors.keys())}"
...
def generate_letter_data(donor_name):
    letter_data = {}
...
def send_letters():
    ...
    write_letters(letter_directory, create_letter_data())

def write_letters(directory, all_letter_data):
    for letter_data in all_letter_data[:]:
        letter_file_name = directory + '{first_name}_{last_name}.txt'.format(**letter_data)
