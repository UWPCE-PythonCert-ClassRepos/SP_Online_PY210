...
def test_header():
    h = H(5, "This is some test")
    file_contents = render_result(h)
    print(file_contents)
    assert file_contents.startswith('<h5>This is some test</h5>')
...
def test_element_indent1():
    ...
    lines = file_contents.split('\n')
    # making sure the opening and closing tags are right.
    assert lines[0] == "<abstract_tag>"
    # this line should be indented by the amount specified
    # by the class attribute: "indent"
    assert lines[1].startswith(Element.indent + "thi")
    assert lines[2] == "</abstract_tag>"
    assert file_contents.endswith("</abstract_tag>")
