...
def test_attributes_2():
    e = P("A paragraph of text", **{'style': "text-align: center", 'id': "intro", 'class': "cool"})
    ...
    assert 'style="text-align: center"' in file_contents
    assert 'id="intro"' in file_contents
    assert 'class="cool"' in file_contents

    # # just to be sure -- there should be a closing bracket to the opening tag
    assert file_contents[:-1].index(">") > file_contents.index('id="intro"')
    assert file_contents[:-1].index(">") > file_contents.index('class="cool"')
...
def test_attributes_ul():
    e = Ul("A paragraph of text", **{'style': "text-align: center", 'id': "intro", 'class': "cool"})
    e.append("More text!")
    ...
    assert "More text!" in file_contents
    assert "\n" in file_contents
...
def test_attributes_li():
    e = Li("A paragraph of text", **{'style': "text-align: center", 'id': "intro", 'class': "cool"})
    e.append("More text!")
...
def test_attributes_h():
    e = H(3, "The text of the header.")
    e.append(" More text!")

    file_contents = render_result(e).strip()
    print(file_contents)  # so we can see it if the test fails
    assert "<h3>The text of the header. More text!</h3>" in file_contents

    e = H(5, "The text of the header.")
    e.append(" More text!")

    file_contents = render_result(e).strip()
    print(file_contents)  # so we can see it if the test fails
    assert "<h5>The text of the header. More text!</h5>" in file_contents

    e = H(3, "The text of the header", **{'style': "text-align: center", 'id': "intro", 'class': "cool"})
    e.append("More text!")
    ...
    # but now the opening tag is far more complex
    # but it starts the same:
    assert file_contents.startswith("<h3 ") # make sure there's space after the p
...
def test_attributes_html_2():
    e = Html("The text of the HTML.")
    e.append(" More text!")
    ...
    assert "The text of the HTML." in file_contents
    assert " More text!" in file_contents
