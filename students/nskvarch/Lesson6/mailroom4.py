...
def thanks(data_base):
    ...
    while True:
        ...
        if response.lower() == "list":
            ...
            for donor_name in data_base:
                if response.lower() == donor_name.lower():
                    data_base[donor_name].append(amount)
                    print_thankyou(donor_name)
                    return data_base

        donor_name = add_new_donor(data_base, response, amount)
        print_thankyou(donor_name)
        return data_base
...
def create_thankyou_all():
    """Takes in the name and donation amount for all donors and creates a thank you text document
...
def add_new_donor(data_base, name, amount):
    """add a donor not in the donor database to the database"""
    data_base.update({name: [amount]})
    return name
...
def main():
    ...
    while True:
        response = input(menu)
        switch_dict = {"1": lambda db=donor_db: thanks(db), "2": run_report, "3": create_thankyou_all}
