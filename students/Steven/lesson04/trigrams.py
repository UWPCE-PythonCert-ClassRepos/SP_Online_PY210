...
def build_text(db, n):  # params of word list and number of words to read
    book = ""
    key = random.choice(list(db.keys()))  # word pair key
    temp = list(key)
    for number in range(n):
        nextWord = (db[key][random.randint(0, (len(db[key]) - 1))])
        temp.append(nextWord)
        key = tuple(temp[-2:])
    story = " ".join(temp)
    story = story.split(".")
    for item in story:
        book += (item[0].capitalize() + item[1:] + ".")
    return book

if __name__ == "__main__":
    filename = r"C:\Users\smar8\PycharmProjects\Python210\lesson04\sherlock_small.txt"
    n = 100  # number of words to read from the file
