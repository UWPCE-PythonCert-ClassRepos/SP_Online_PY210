...
# Support Functions
def get_donor_names():
    report = ["Names of Donors: "]
    for x in donor_donations:
        report.append(x)
    print(report)
    return report

def send_a_thank_you_prompt():
    print('\n----- Send a Thank You -----')
    ...
    send_a_thank_you(user_action_TY, donor_name_list)

def send_a_thank_you(user_action_TY, donor_name_list):
    if user_action_TY == 'list':
        get_donor_names()
        send_a_thank_you_prompt()
    else:
        donation = input('Donation Amount ($): ')
        new_donation(user_action_TY, donor_name_list, donation)

def new_donation(user_action_TY, donor_name_list,donation):
    try:
    ...
    return Email_Thanks.format(user_action_TY, donation)
...
def send_all_letters():
    for donor, data in donor_donations.items():
        ...
        with open(f"{name}.txt", 'w+') as fileout:
            ...
            fileout.write(f"Thank you for your kind donation of ${total_donation}. We will use this money for good "
...
switch_action_dict = {'1': send_a_thank_you_prompt,
                      '2': create_a_report,
                      '3': send_all_letters,
                      '4': quit_program}
