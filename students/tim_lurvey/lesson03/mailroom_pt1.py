...
def send_thank_you():
    ...
    if 'list' == request.strip().lower():
        ...
        try:
            ...
            print("The value you entered is not a valid donation\n>>> ")
            return False
        # add the donation to their existing amount
        add_donation(name=request, ammount=float(donation))
        # return the email string
        return compose_email(name=request, donation=float(donation), data=myData.get(request))
