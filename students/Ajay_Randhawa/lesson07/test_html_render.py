...
def test_one_line_tag_append():
    """
    Yoou should not be able to append content to a OneLineTag
    """
...
def test_br():
    """
    tests that the input text is rendered
    """
    br = Br()
...
def test_li():
    ...
    li.append(" some more content")
    file_contents = render_result(li)
    print(file_contents)
    assert '<li>\n' in file_contents
    assert 'list Li\n' in file_contents
    assert 'some more content\n' in file_contents
    assert '</li>' in file_contents


def test_header():
    h1 =  H(1,"the text of header 1")
    h2 = H(2, "the text of header 2")
    """
    tests that the input text is rendered
    """
    file_contents = render_result(h1)
    print(file_contents)
    assert  "<h1>the text of header 1</h1>" in file_contents

    file_contents = render_result(h2)
    print(file_contents)
    assert "<h2>the text of header 2</h2>" in file_contents
...
def test_meta():
    ...
    print(file_contents)
    assert  "<meta charset=" in file_contents
...
def test_multiple_indent():
    ...
    for i in range(3):  # this needed to be adapted to the <DOCTYPE> tag
        assert lines[i + 1].startswith(i * str(" "*Element.indent) + "<")

    assert lines[4].startswith(3 * str(" "*Element.indent) + "some")
...
def test_element_indent1():
    ...
    assert lines[1].startswith(str(" "*Element.indent) + "thi")
    assert lines[2] == "</html>"
    assert file_contents.endswith("</html>")
