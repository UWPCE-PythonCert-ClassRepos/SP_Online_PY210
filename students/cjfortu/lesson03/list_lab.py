...
def ask():
    # option for other Series to take default list or execute Series1
    ask = input("Do you want the default list? ('yes' or 'no'): ").lower()
    if ask == 'yes':
...
(Bonus: Multiply the list times two. Keep asking until a match is found. Once found, delete \
all occurrences.)
    """
...
list (making the fruit all lowercase).
For each “no”, delete that fruit from the list.
For any answer that is not “yes” or “no”, prompt the user to answer with one of those two \
values (a while loop is good here)
Display the list.
    """
...
def Series4():
    ...
    Make a new list with the contents of the original, but with all the letters in each item \
    reversed.
