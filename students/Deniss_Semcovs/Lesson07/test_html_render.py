...
#     tests that you can add another element and still render properly
#     """
    e = Head("this is some text03")
    e.append("and this is some more text03")

    file_contents = render_result(e).strip()

    assert("this is some text03") in file_contents
    assert("and this is some more text03") in file_contents
...
def test_title():
    ...
    # test ability to generate on text line
...
def test_ul():
    ul = Ul("test text")
    ul.append("looks good")
    file_contents = render_result(ul).strip()
    assert("test text") in file_contents
    assert("looks good") in file_contents
