...
def test_render_element():
    ...
    # This uses the render_results utility bove
...
def test_one_liner_attr():
    t = Title("A Title of text", style="text-align: center", id="intro")
    ...
    #       properly rendered, so we don't need to test that here.
    #       so using only a "P" tag is fine
    assert "A Title of text" in file_contents
    # but make sure the embedded element's tags get rendered!
    # first test the end tag is there -- same as always:
    assert file_contents.endswith("</title>")
    ...
    # but now the opening tag is far more complex
    # but it starts the same:
    assert file_contents.startswith("<title ") # make sure there's space after the p
...
def test_Li_attr():
        e = Li(id="TheList", style="line-height:200%")
        file_contents = render_result(e)
        print(file_contents)
        assert file_contents == '<li id="TheList" style="line-height:200%">\n</li>\n'
...
def test_SelfClosingTag_indent():
    ...
    print(file_contents)
    assert file_contents == "  <title></title>\n"
...
def test_SelfClosingTag_indent():
    ...
    print(file_contents)
    assert file_contents == "     <title>This is a Title</title>\n"
