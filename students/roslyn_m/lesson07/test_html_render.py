...
def test_head():
    head = Head("HEY! this is some text")
    head.append("HEY HEY! and this is some more text")

    file_contents = render_result(head).strip()

    assert("HEY! this is some text") in file_contents
    assert("HEY HEY! and this is some more text") in file_contents
...
def test_title():
    title = Title("HEY! this is the title")
    file_contents = render_result(title).strip()
    print(file_contents)
    assert ("HEY! this is the title") in file_contents
...
def test_ul():
    ul = Ul("this is the first bullet point", id="TheList", style="color: red")
    file_contents = render_result(ul).strip()
    print(file_contents)
    assert("this is the first bullet point") in file_contents
    assert file_contents.startswith("<ul")
    assert file_contents.endswith("</ul>")


def test_li():
    li = Li("this is the second bullet point", id="TheList", style="color: red")
    file_contents = render_result(li).strip()
    print(file_contents)
    assert("this is the second bullet point") in file_contents
    assert file_contents.startswith("<li")
    assert file_contents.endswith("</li>")


def test_header():
    header = H(1, "PythonClass - Class 6")
    file_contents = render_result(header).strip()
    print(file_contents)
    assert file_contents.startswith("<h1>")
    assert file_contents.endswith("</h1>")
    assert "<h1>PythonClass - Class 6</h1>"

def test_meta():
    mt = Meta()
    file_contents = render_result(mt)
    print(file_contents)
...
def test_multiple_indent():
    ...
    for i in range(2):  # this needed to be adapted to the <DOCTYPE> tag
