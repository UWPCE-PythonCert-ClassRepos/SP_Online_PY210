...
# -----------------------------------------------------------------------------
# Lesson05
# Mailroom Part 3 (mailroom3.py)
# Steve Long 2020-11-02 | v3
#
# Requirements:
# =============
#
#   [1] Update code to handle errors.
#   [2] Use comprehensions to clean up your code a bit.
#
# Assumptions:
# ============
#
...
# ===============
#
#   [1] Most of v2 of this application already handles input from user that is
#       not a proper command or value that is not appropriate for the context.
#       However, existing try-except blocks were updated with more specific
#       Exception subclasses and new blocks were added to handle out-of-band
#       raw user input and file operation failures.
#
#   [2] There was already at least one use of list comprehension in v2 but
#       several more loops were substituted where the chief result(s) was a
#       (were) side-effect(s).
#
#   These and several clean-up changes are marked in the code under change 002.
#   Checked with linter flake8.
#
# Script Usage:
# =============
#
...
def report_header(col_names, col_widths):
    ...
    HIST: 002/Note: List comprehension attempted but this makes the code
              larger and more complicated - which is not the goal.
          000/New
    """
    s = "\nDonor Report"
...
def show_mailroom_ui():
    ...
    while (cmd != ":Q"):
        ...
        if (response in valid_cmds):
            ...
            cmd = ":O"  # Valid command callable by routine, not user
            state["name"] = response
    print("\nExiting {}".format(ui_title))
