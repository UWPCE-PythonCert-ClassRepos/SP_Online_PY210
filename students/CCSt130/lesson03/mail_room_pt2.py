...
Edited Sun Aug 4 17:31:12 2019
...
def main():
    ...
    while True:
        ...
        # This will unintentionally print primary keys w/o elif '4'
    ...
    while True:
        ...
        if amt_input.isdigit():
            ...
            if new_amt > 0 and new_amt < 1000000000: # n < billion
                ...
                # Replaces all entries for existing donor
                # all_donations[a_donor] = {dt: new_amt}
                break
            else:
                print()
...
def prn_an_email(all_donations):
...
def prn_ltrs(all_donations):
    ...
    for key in all_donations:
        ...
        # Preferred method
        with open(test_file, "w") as donor_ltr:
        # donor_ltr = open(test_file, "w")
        # Begin letter
            donor_ltr.write('\n')
            donor_ltr.write(the_date)
            donor_ltr.write('\n\n\n\n')
            # Dict is a template 
            donor_email = {'Salutation': "Dear {},\n\n",
                           'Body': "We are thrilled to have been the recipient of your generosity! \
            ...
            donor_ltr.write(donor_email['Salutation'].format(a_donor))
            donor_ltr.write(donor_email['Body'])
            ...
            donor_ltr.write(donation_format['Heading1'])
            # Print column labels 
            donor_ltr.write('{:<11} {:<36} {}'.format(donation_format['Column1'], donation_format['Column2'], donation_format['Column3']))
            donor_ltr.write('\n')
            donor_ltr.write(66 * '-')
            donor_ltr.write('\n')
            ...
            # Formatting fix tba
            for gift_dt, gift_amt in sorted(all_donations[a_donor].items()):
                donor_ltr.write('{:<13} {:<36} ${:>14,.2f}'.format(gift_dt, a_donor, gift_amt))
                donor_ltr.write('\n')

            # Signature block 
            donor_ltr.write(donor_email['Sig_block'])
            # Not needed
            # donor_ltr.close() # Close file
            print()
...
def prn_donor_avg(all_donations):
    ...
    for key in all_donations:
        ...
        donation_dict = Counter(all_donations[key].items())
        ...
        all_donations[key]['Donation Count'] = donation_ct
        # Sum values in subkeys and assign to a name
        donation_ttl = sum(all_donations[key].values())
        # Add subkey and value (TTL) to data structure
        all_donations[key]['Total Donations'] = donation_ttl
        # Calculate average and assign to a name
        donation_avg = (donation_ttl / donation_ct)
        # Add subkey and value (AVG) to data structure
        all_donations[key]['AVG Donation'] = donation_avg

    # Sort dict by 'Total Donations' descending
    lets_sort = sorted(all_donations.items(), key = lambda x: x[1]['Total Donations'], reverse = True) 
    
    # Change it back to a dict
    all_donations_sorted = dict(lets_sort)
    
    # Iterate over sorted dict    
    for key in all_donations_sorted:
        temp_donor = key
        # Print formatted TTL, Count, AVG
        print('{:<36} {:>} {:>14,.2f} {:>3} {:>11} {:>13,.2f}'.format(temp_donor, usd, all_donations_sorted[key]['Total Donations'], \
              all_donations_sorted[key]['Donation Count'], usd, all_donations_sorted[key]['AVG Donation']))
