...
def test_attributes():
    e = P("some content", id="intro", style="text-align: center")
...
#You could also special-case this in your code – so your users could use “clas” with one s, and you could tranlate it in the generated html. Or even both!
    ...
    assert file_contents[:-1].index(">") > file_contents.index('style="text-align: center"')
...
def test_list():
    ...
    assert file_contents.startswith('<ul style="list-style-type:circle;"')
    assert ('<li>' + '\n' + ' ' * 8 + 'Item 1' + '\n') in file_contents
    assert ('<li>' + '\n' + ' ' * 8 + 'Item 2' + '\n') in file_contents
    assert file_contents.endswith('</ul>')

def test_h():
    h = H(3, "Text header level 3")
    file_contents = render_result(h).strip()
    assert file_contents == '<h3>Text header level 3</h3>'
