...
def test_unordered_list():
    ul = Ul(style="line-height:200%", id="TheList")
...
def test_list_element():
    ...
    file_contents = render_result(li1).strip()
    assert '<li>' in file_contents
    assert 'The first item in a list' in file_contents
    assert '</li>' in file_contents
    file_contents = render_result(li2).strip()
...
def test_multiple_indent():
    ...
    for i in range(3):  # this needed to be adapted to the <DOCTYPE> tag
        assert lines[i + 2].startswith(i * 4*" " + "<")
    assert lines[5].startswith(3 * 4*" " + "some")
...
def test_element_indent1():
    ...
    assert lines[1].startswith(4*" " + "thi")
    assert lines[2] == "</html>"
    assert file_contents.endswith("</html>")