...
def verify_donor(name):
    #This function will check if the donor exists. If not there, it will
    #add to the donor_db.
    if name in donor_db:
        return
    donor_db[name] = []
...
def add_donation(name, amount):
    ...
    donor_db.get(name).append(amount)
...
def send_a_thank_you():
    ...
    #amount to the list associated with the donor. At the end, a 'Thank You' letter
    ...
    #For now, the letter will print to the terminal.
    ...
    print(letter_to_donor(name,float(amount), 1))
...
def create_a_report():
    ...
    for name, amounts in donor_db.items():
        summary.append((name, sum(amounts), len(amounts)))
