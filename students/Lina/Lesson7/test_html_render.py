...
def test_attributes_2():
    #test the one-line tag did not break with attributes included
    e = Title("This is a Title", style="text-align: center", id="anId")

    file_contents = render_result(e).strip()

    assert('style="text-align: center"') in file_contents
    assert('id="anId"') in file_contents
    assert(">This is a Title</title>") in file_contents
    assert file_contents.startswith("<title ")
...
def test_indent_OneLineTag():
    # Tests whether the OneLineTag indents correctly
    # <!DOCTYPE html>
    # <html>
    #     <title>This is a Title</title>
    # </html>
    html = Html()
    html.append(Title("This is a Title"))
    ...
    assert lines[2].startswith(Element.indent + "<title>This is a Title</title>")
    assert file_contents.endswith("</html>")

def test_indent_SelfClosing():
    """
      Tests whether the SelfClosing indents correctly
      Uses the logic from test_multiple_indent() and adds the Hr()
      Just needs to check the indentation for Hr()
    """
    body = Body()
    body.append(P("some text"))
    body.append(Hr())
    ...
    print(file_contents)
    lines = file_contents.split("\n")
    assert lines[6].startswith(2 * Element.indent + "<hr />")
