...
def donor_list():
    #This function will return a list with all donors sorted by name.
    return sorted(donor_db.keys())
...
def donors_summary():
    #This function will return a list with summary of each donor in a tuple
    # [(name, total donated, donation count, average amount)]
    summary = [(name, round(sum(amount), 2), len(amount), round(sum(amount)/len(amount), 2)) for (name, amount) in donor_db.items()]
...
def create_a_report():
    ...
    for col in summary:
        print(row(name=col[0], total_donated=col[1], gift_count=col[2], avg_amount=col[3]))
    print()
