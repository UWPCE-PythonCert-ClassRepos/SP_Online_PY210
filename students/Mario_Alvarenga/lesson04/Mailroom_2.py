...
"""
a_donors = [('Lionel Messi',[12450, 49563, 65897, 58690]),
          ('Cristiano Ronaldo',[65000, 98520]),
          ('Robert Lewandowski',[85005,48912,856940]),
          ('Neymar Dos Santos',[26598, 74158]),
          ('Karim Benzema',[865204, 58740, 15069])]
...
def f_add_donation(donor_name, donation_amount):
    ...
    a_donors[donor_index][1].append(donation_amount)
...
def f_execute_thank_you(name):
    ...
    # avoiding dict switch because it would create too small scope methods
...
"""
#LESSON 3 function - Lesson 4 changes user selection and execution
def f_write_thank_you():
    # Add donation for new or existing donor and compose 'Thank You' message
    # Get current list of donors
    donor_names = f_get_donor_names()
    while True:
        print('\nType "list" for a full list of donors.\nType "return" to return to the main menu.\n')
        name = input('Enter donor name:\t')
        if name.lower() == 'list': # List donors
            print('\nCurrent list of donors:\n')
            print('\n'.join(donor_names))
        elif name.lower() == 'return': # Return to main menu
            return
        else:
            #
            #Lesson 4 - Since we have to append data structure, append when you have ALL the data to append a dictionary
            #
            ...
            if amount.lower() == 'return':
                #
                #LESSON 3 - Since we're using a dictionary, no need to do section below
                #
...
def f_send_all_letters():
    ...
    for name in dict_donors.keys():
        ...
        #
        file_full_path = '{path}{subfolder}{file}.txt'.format(path=os.path.normpath(os.path.expanduser("~/Desktop")),
                                                              subfolder=os.path.normpath('/'), file=name)
        #
