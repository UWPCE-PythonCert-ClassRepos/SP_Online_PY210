...
def test_list_invoked_list():
    with patch('mailroom4.report') as report_mock:
        list_invoked('list')
        assert report_mock.called == True
...
def test_valid_amt():
    with patch('mailroom4.ty_logic') as tylogic_mock:
        amt_logic('name', 43)
        assert tylogic_mock.called == True

def test_invalid_amt():
    with patch('sys.stdout', new=StringIO()) as fake_output:
        amt_logic('name', 'blah')
        assert fake_output.getvalue().strip() == '--->Not a valid amount, please try your submission again'
...
def test_switch_bad1():
    with patch('sys.stdout', new=StringIO()) as fake_output2:
        main_switch('dog')
        assert fake_output2.getvalue().strip() == '----> Invalid Selection: Please input a number 1-4'

def test_switch_bad2():
    with patch('sys.stdout', new=StringIO()) as fake_output3:
        main_switch(5)
        assert fake_output3.getvalue().strip() == '----> Invalid Selection: Please input a number 1-4'
...
def test_file_creation():
    ...
    assert os.path.isfile(testdest) == True
