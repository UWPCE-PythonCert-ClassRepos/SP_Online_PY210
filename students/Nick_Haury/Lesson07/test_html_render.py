...
def test_self_closing_and_one_line_indent():
    html = Html("test self closing")
    html.append(Br(width="200"))
    html.append(H(2, "here is some content"))
    file_contents = render_result(html).strip()
    print(file_contents)
    lines = file_contents.split('\n')
    assert lines[3].startswith(Element.indent + '<br')
    assert lines[3].endswith(' />')
    assert lines[4].startswith(Element.indent + '<h2')
    assert lines[4].endswith('</h2>')
